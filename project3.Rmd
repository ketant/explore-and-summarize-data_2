...
ld.s.cr_grade <- subset(ld, 
                               BorrowerState %in% c("CA","TX",
                                                    "NY","IL","FL"),                             
...
ggplot(data=ld.plt1, 
  ...
  scale_x_discrete(factor(ld.plt1$ListingCategory..numeric.), 
                     labels=c("Not Available","Debt Consolidation", 
                              "Home Improvement","Business",
                              "Personal Loan","Student Use",
                              "Auto","Other"), 
                     limits=c("Not Available","Debt Consolidation", 
                              "Home Improvement","Business",
                              "Personal Loan","Student Use",
                              "Auto","Other"))+ 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ...
  scale_y_log10( breaks = trans_breaks("log10", function(x) 10 ^ x),
                labels = trans_format("log10", math_format(10^.x))) + 
  facet_wrap(~EmploymentStatus, ncol=2,scales="free_y")
...
ggplot(data=subset(ld, 
                   !(ProsperRating..Alpha.%in% c("D","E","HR","")) & 
                     DebtToIncomeRatio<1.0 & 
                     LoanStatus %in% c("Chargedoff","Defaulted") &
                     EmploymentStatus %in% c("Employed",
                                             "Full-time","Self-employed")
                   ), 
          aes(x=LoanOriginalAmount )) + 
  xlim(2000,10000) +
 geom_bar(position = "dodge") + 
    facet_grid(ProsperRating..Alpha.~EmploymentStatus, scales="free_y")
```

### Description Three

Final Plot 3 shows a subset of loans data which were defaulted or charged off for employment status Employed, Full-time or Self-Employed, good Prosper Rating Alpha (AA,A,B,C) and Debt to income ratio of less than 1.0.  
...
head(
  ...
  subset(ld, 
         LoanStatus %in% c("Chargedoff","Defaulted") &
          !(ProsperRating..Alpha.%in% c("D","E","HR","")) & 
         DebtToIncomeRatio<1.0,
         select=c(LoanOriginalAmount, EmploymentStatus,ProsperRating..Alpha.)
         )
  , vars=c("LoanOriginalAmount","EmploymentStatus","ProsperRating..Alpha."))
  , desc(freq)))
```

We can see that most deafult amount ranges from 15000,10000,5000, 4000.  
People with Prosper Rating C and loan amounts of 15000 are most likely to default.  
